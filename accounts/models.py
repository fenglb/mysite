...
class CustomUserManager( BaseUserManager ):
    ...
    def create_superuser( self, username, surname, email, identify, phone_number, password ):
        ...
        orgnization.name = u"厦门大学"
        orgnization.department = u"化学化工学院"
        orgnization.address = u"厦门市思明区思明南路220号"
        orgnization.save()
...
class Orgnization( models.Model ):
    name       = models.CharField( verbose_name=u'单位名称', max_length=250 )
    department = models.CharField( verbose_name=u'部门', max_length=100 )
    address    = models.CharField( verbose_name=u'地址', max_length=250 )
...
class PersonInCharge( models.Model ):
    surname0      = models.CharField( unique=True, verbose_name=u'姓名', help_text=u'用户的真实姓名', max_length=30)
    phonenumeric = RegexValidator(r'^\+?1?\d{9,15}$', message=u'格式为05922186874或者手机号码')
    phone_number0 = models.CharField( verbose_name=u'电话号码', help_text=u'格式为05922186874或者手机号码', max_length=18, default='05920000000' )
    email0 = models.EmailField( verbose_name=u'邮箱', help_text=u'有效邮箱，用于认证通知', max_length=255, default='unkown@xmu.edu.cn')
    titles_choice = (
        ('PI', u'课题组负责人'),
        ('FL', u'经费负责人'),
        ('OL', u'公司领导')
    )
    titles = models.CharField( verbose_name=u'职务', max_length=3, choices=titles_choice, default='PI')
    orgnization = models.ForeignKey( Orgnization, verbose_name=u'单位', null=True, blank=True )

    def __str__(self):
        return self.surname0.encode('utf-8')

class CustomUser( AbstractBaseUser, PermissionsMixin ):

    alphanumeric = RegexValidator(r'^[0-9a-zA-Z]*$', message=u'只有[0-9a-z-A-Z]字符可以.')
    username     = models.CharField( verbose_name=u'用户名', help_text=u'纯数字和字母', unique=True, max_length=30, validators=[alphanumeric])
    surname      = models.CharField( verbose_name=u'姓名', help_text=u'用户的真实姓名', max_length=30)
    identify     = models.CharField( verbose_name=u'学号/教工号', help_text=u'厦大学生号或者教工号，非厦大学生或者教师不用填', max_length=30, unique=True, null=True, blank=True )
    phonenumeric = RegexValidator(r'^\+?1?\d{9,15}$', message=u'格式为05922186874或者手机号码')
    phone_number = models.CharField( verbose_name=u'电话号码', help_text=u'格式为05922186874或者手机号码', max_length=18, default="05920000000" )
    position_choice = (
                ('student', u"厦大学生"),
                ('staff', u"厦大教师"),
                ('visit', u"其他"),
    ...
    def get_position_name(self):
        return dict(self.position_choice)[self.position].encode('utf-8')
    ...
    is_expired.short_description= u'过期'
